...
const defaultForm: BrandFormData = { name: "", description: "", image: null };

export default function BrandsPage() {
  const { data, isLoading } = useGetBrands();
  const { mutate: create, isPending: creating } = useCreateBrand();
  const { mutate: update, isPending: updating } = useUpdateBrand();
  const { mutate: toggle } = useToggleBrand();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editing) {
      update({ id: editing._id, formData: fd }, { onSuccess: () => setModalOpen(false) });
    } else {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{brands.length} total</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {brands.length === 0 ? (
                ...
                brands.map((brand) => (
                  <tr key={brand._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        {brand.image ? (
                          <img src={brand.image} alt={brand.name} className="w-8 h-8 rounded-lg object-cover border border-gray-100" />
                        ) : (
                    ...
                    <td className="px-4 py-3.5">
                      <Badge label={brand.isActive ? "Active" : "Inactive"} variant={brand.isActive ? "success" : "default"} />
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(brand)} className="text-gray-400 hover:text-gray-700 transition-colors">
                          <Pencil size={14} />
                        </button>
                        <button
                          onClick={() => toggle(brand._id)}
                          className={`transition-colors ${brand.isActive ? "text-green-500 hover:text-red-500" : "text-gray-400 hover:text-green-500"}`}
                        >
                          <Power size={14} />
                        </button>
                      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit Brand" : "Add Brand"} size="sm">
